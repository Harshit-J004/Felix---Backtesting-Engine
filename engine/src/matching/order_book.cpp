...
namespace felix {
    ...
    void OrderBook::cancel_order(uint64_t order_id) {
        ...
        if (it != orders_.end()) {
            it->second.active = false; // Soft delete or just remove
            orders_.erase(it);
        }
    }
    ...
    std::vector<Fill> OrderBook::check_fills(double market_price) {
        ...
        // Naive iteration (O(N)) - For "Simple+Best" backtester this is fine.
        // For HFT optimization, we would use price-time priority queues.
